...
import { useGetAssessmentListQuery } from "./adminApiSlice";
...
export default function AdminAssessmentsList() {
  ...
  // ---------------------- API CALL ----------------------
  const { data: apiData, isLoading, isError, refetch } = useGetAssessmentListQuery({
    page,
  ...
  const { data: allData } = useGetAssessmentListQuery({
    page: 1,
    ...
    <div className="student-list-table-wrapper">
      ...
      <TableContainer component={Paper} className="student-table-container">
        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
          <h2 className="table-title">Assessments List</h2>
          <div> 
        ...
        <Table className="student-table">
          <TableHead>
            <TableRow>
              ...
              <TableCell className="header-cell">Testgorilla Code </TableCell>
            </TableRow>
          </TableHead>
            ...
            {isLoading
              ...
              apiData?.data?.map((s, i) => (
                <TableRow key={s.id} className="row">
                  ...
                  <TableCell className="grade">{s.main_career}</TableCell>
                  <TableCell className="grade">{s.sub_career}</TableCell>
                  <TableCell className="grade">{s.name}</TableCell>
      ...
      >
        <div style={{ width: 300, padding: 20 }}>
          <Typography variant="body" sx={{ color: '#333', marginBottom: 2, display: 'block' }}>Filter Assessment</Typography>
